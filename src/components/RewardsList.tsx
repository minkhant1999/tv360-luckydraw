...
function RewardItem({ reward, isSelected, disableSelect, onSelect }: RewardItemProps) {
  return (
    <button
      ...
      className={`group relative w-full min-h-[65px] border-0 bg-transparent pr-3 text-left ${disableSelect ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'
        }`}
    ...
    >
      <div className="pointer-events-none absolute left-1.5 top-[31px] z-20 flex h-[60px] w-[60px] -translate-y-1/2 items-center justify-center">
        <img
          src={reward.image}
          alt=""
          className={`h-[62px] w-[64px] object-contain drop-shadow-md ${disableSelect ? 'grayscale' : ''
            }`}
        />
      </div>

      <div
        className={`ml-[28px] flex-1 ${isSelected && !disableSelect
          ? 'rounded-xl p-[2.28px] bg-[radial-gradient(ellipse_32%_22%_at_50%_0%,#ffc400_0%,#ff080e_62%,#ff080e_100%)]'
          : ''
          }`}
      >
        <div
          className={`relative flex min-h-[56px] items-center justify-between overflow-hidden rounded-xl py-2 pl-[34px] pr-3 ${disableSelect
        ...
        >
          ...
          <div className="relative z-10 min-w-0 flex-1 left-2">
            <p
              className={`font-supreme-bold text-sm m-0 leading-tight ${disableSelect
                ? 'text-[#a0a0a0]'
                : isSelected
                  ? 'text-[#ae1700]'
                  : 'text-[#232323]'
            ...
            <p
              className={`font-ui font-medium text-[11px] m-0 mt-0.5 ${disableSelect ? 'text-[#bdbdbd]' : 'text-[#8d8c8c]'
                }`}
            >
              {reward?.quantity}/{reward?.initialQuantity}
            </p>
          </div>
          {isSelected && !disableSelect && (
            <img
